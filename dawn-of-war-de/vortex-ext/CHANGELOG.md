...
### Fixed
- **FOMOD archives are no longer intercepted by the generic installer.** If a mod archive contains `fomod/ModuleConfig.xml` and `fomod/info.xml`, the extension now declines it so Vortex can show the built-in installer options UI.

### Changed
- Extension metadata updated to describe FOMOD-aware behavior.
