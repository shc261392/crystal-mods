#!/usr/bin/env bash
# Build Army Painter and package it as a Vortex-installable zip.
#
# The zip lays the plugin at BepInEx/plugins/ArmyPainter.dll — the Battlesector
...
dotnet build "$HERE/ArmyPainter.csproj" -c Release >/dev/null

DLL="$HERE/bin/Release/ArmyPainter.dll"
...
cp "$DLL" "$STAGE/BepInEx/plugins/ArmyPainter.dll"
